...
\begin{document}
  ...
  \begin{frame}{Результаты 1 года}
    \begin{center}
      \todo{Скриншоты с сердцем!}

      % TODO почему съезжает картинка на <1> слайде?
      ...
      \only<4>{ Графический интерфейс \todo{Скриншот GUI!} \\ \includegraphics[height=0.7\textheight]{screen-2} }
    \end{center}
  \end{frame}
